"""
Day 59 Mini Project - Validated Blog API

Day 58's Blog API, upgraded with:
    - Stricter Pydantic validation (EmailStr, min_length, age range).
    - Explicit HTTPException error handling with clear status codes.
...
Test - invalid request (wrong types -> 422):
    curl -X POST http://127.0.0.1:8000/users \
        -H "Content-Type: application/json" \
        -d '{"name": 123, "email": "not-an-email", "age": "hello"}'

Test - post for a user_id that doesn't exist (-> 404):
...
app = FastAPI(title="Day 59 - Validated Blog API")
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    # By the time we get here, Pydantic has already validated `user`:
    # name is non-empty, email looks like an email, age is 1-150.
    # A malformed body never reaches this line - FastAPI returns 422
...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db)):
    ...
    # 404 instead of writing an orphaned foreign key.
